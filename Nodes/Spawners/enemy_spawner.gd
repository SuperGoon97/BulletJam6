...
const ENEMY_TWIN = preload("res://Nodes/Enemies/enemy_twin.tscn")
...
Gv.ET.BASIC_TWIN:ENEMY_TWIN
}

func spawn_enemy(speed:float , direction:Vector2 , type:Gv.ET , score):
	var new_enemy :Enemy = enemy_dictionary[type].instantiate()
	new_enemy.speed = speed
	new_enemy.direction = direction
	new_enemy.score_value = score
	add_child(new_enemy)
	return new_enemy
