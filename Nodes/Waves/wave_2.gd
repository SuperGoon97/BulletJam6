class_name WaveTwo extends Wave

func _init() -> void:
	number_of_spawns = 20
	position_sequence = [0 , 6 , 7 , 9 , 0 , 6 , 7 , 9 , 0 , 6 , 7 , 9 , 0 , 6 , 7 , 9 , 7 , 9 , 8 , 10]
	for n in 16:
		if position_sequence[n] == 9:
			cooldown_sequence.append(2)
		else:
			cooldown_sequence.append(0)
		if n%2 == 0:
			direction_sequence.append(Vector2(3.0 , 1.0))
		else:
			direction_sequence.append(Vector2(-3.0 , 1.0))
